...
TransversityVarsB::TransversityVarsB(bool InGeV) {
  IsInGev = InGeV;
  ...
  _FourMomentumTransfer = &FourMomentumTransfer;

  _FS_PSum = &FS_PSum;
  _ChargedFS_PSum = &ChargedFS_PSum;
...
bool TransversityVarsB::HandleStdHepParticle(UInt_t &StdHepPosition,
                                             Int_t &StdHepPdg,
                                             Int_t &StdHepStatus,
                                             Double_t *&StdHepP4) {
  ...
  if (StdHepPdg >= 1000000000) {
    return false;
  }  // Should catch nuclear PDGs
  if (StdHepPdg >= 2000000000) {
    return false;
  }  // GENIE psuedo particles
...
void TransversityVarsB::Finalise() {
  ...
  TVector3 Muon_Pt_MeV = TransversityUtils::GetVectorInTPlane(
      Muon.FourMomentum.Vect(), MuonNeutrino.FourMomentum.Vect());
  ...
  // deltaphiT
  deltaphiT_HMProton_deg =
      GetDeltaPhiT(Muon.FourMomentum.Vect(), HMProton.FourMomentum.Vect(),
                   MuonNeutrino.FourMomentum.Vect()) *
      RadToDeg;

  // Deltap
  Deltap_HMProton_MeV =
      (Muon.FourMomentum + HMProton.FourMomentum - MuonNeutrino.FourMomentum -
  ...
  // deltapt
  deltapT_HMProton_MeV =
      GetDeltaPT(Muon_Pt_MeV, HMProtonPt_MeV, MuonNeutrino.FourMomentum.Vect());

  // deltaalphaT
  deltaalphaT_HMProton_deg = GetDeltaAlphaT(Muon_Pt_MeV, HMProtonPt_MeV,
                                            MuonNeutrino.FourMomentum.Vect()) *
                             RadToDeg;
  // deltap_TT
  if ((HMPion.Momentum > 1E-3) && (HMProton.Momentum > 1E-3)) {
    deltap_TT = TransversityUtils::GetDeltaPTT(
        Muon.FourMomentum.Vect(), HMPion.FourMomentum.Vect(),
        HMProton.FourMomentum.Vect(), MuonNeutrino.FourMomentum.Vect());

    HMProtonPion_3Mom_MeV =
        HMProton.FourMomentum.Vect() + HMPion.FourMomentum.Vect();

    TVector3 HMProtonPionPt_MeV = GetVectorInTPlane(
        HMProtonPion_3Mom_MeV, MuonNeutrino.FourMomentum.Vect());

    deltaphiT_HMProtonPion_deg =
        GetDeltaPhiT(Muon.FourMomentum.Vect(), HMProtonPion_3Mom_MeV,
                     MuonNeutrino.FourMomentum.Vect()) *
        RadToDeg;
    deltapT_HMProtonPion_MeV = GetDeltaPT(Muon_Pt_MeV, HMProtonPionPt_MeV,
                                          MuonNeutrino.FourMomentum.Vect());
    deltaalphaT_HMProtonPion_deg =
        GetDeltaAlphaT(Muon_Pt_MeV, HMProtonPionPt_MeV,
                       MuonNeutrino.FourMomentum.Vect()) *
        RadToDeg;

    Deltap_HMProtonPion_MeV =
        (Muon.FourMomentum + HMProton.FourMomentum + HMPion.FourMomentum -
  ...
  ChargedHadrMass =
      sqrt(ChargedFS_ESum * ChargedFS_ESum - ChargedFS_PSum.Mag2());
...
void TransversityVarsB::Reset() {
  ...
  FourMomentumTransfer = TLorentzVector(0,0,0,0);
...
void TransversityVarsB::AddBranches(TTree *tree) {
  ...
  tree->Branch("FourMomentumTransfer", &_FourMomentumTransfer);
  ...
  tree->Branch("FS_ESum", &FS_ESum, "ChargedHadrMass/D");
...
TransversityVars::TransversityVars(bool InGeV, Int_t NThresh,
                                   Int_t *Threshs_MeV, TString generatorName)
    : TransversityVarsB(InGeV) {
...
void TransversityVars::HandleCPion(TLorentzVector &StdHepPTLV,
  ...
  if ((pdg == 211) && ((StdHepPTLV.E() * (IsInGev ? 1000.0 : 1.0)) >
...
bool TransversityVars::HandleStdHepParticle(UInt_t &StdHepPosition,
                                            Int_t &StdHepPdg,
                                            Int_t &StdHepStatus,
                                            Double_t *&StdHepP4) {
  if (!TransversityVarsB::HandleStdHepParticle(StdHepPosition, StdHepPdg,
...
void TransversityVars::Finalise() {
  ...
  ReconNuEnergy = GetReconNuEnergy(MuonNeutrino.FourMomentum.Vect(),
                                   Muon.FourMomentum, HMProton.FourMomentum);
  ReconTargetMass =
      GetReconTgtMass(ReconNuEnergy, Muon.FourMomentum, HMProton.FourMomentum);
  ...
  // Muon
  Muon_Pt_MeV = TransversityUtils::GetVectorInTPlane(
      Muon.FourMomentum.Vect(), MuonNeutrino.FourMomentum.Vect());
  ...
  StruckNucleon_3Mom_Recon_MeV =
      (Muon.FourMomentum + FirstProton.FourMomentum - MuonNeutrino.FourMomentum)
          .Vect();
  // PreFSINucleon_3Mom_Recon
  PreFSINucleon_3Mom_Recon_MeV =
      (MuonNeutrino.FourMomentum + StruckNucleon.FourMomentum -
       Muon.FourMomentum)
          .Vect();
  ...
  // deltaphiT
  deltaphiT_FirstProton_deg =
      GetDeltaPhiT(Muon.FourMomentum.Vect(), FirstProton.FourMomentum.Vect(),
                   MuonNeutrino.FourMomentum.Vect()) *
      RadToDeg;

  deltaphiT_HMTrackable_deg =
      GetDeltaPhiT(Muon.FourMomentum.Vect(), HMTrackable.FourMomentum.Vect(),
                   MuonNeutrino.FourMomentum.Vect()) *
      RadToDeg;

  // deltapT
  deltapT_FirstProton_MeV = GetDeltaPT(Muon_Pt_MeV, FirstProtonPt_MeV,
                                       MuonNeutrino.FourMomentum.Vect());

  // Deltap
  Deltap_FirstProton_MeV =
      (Muon.FourMomentum + FirstProton.FourMomentum -
       MuonNeutrino.FourMomentum - StruckNucleon.FourMomentum);
  ...
  deltaalphaT_FirstProton_deg =
      GetDeltaAlphaT(Muon_Pt_MeV, FirstProtonPt_MeV,
                     MuonNeutrino.FourMomentum.Vect()) *
      RadToDeg;
  ...
  //                      FS Particle Stuff
  //******************************************************************************
  auto GetLastNonZeroIndex =
      [](TLorentzVector(&tlvs)[kMaxFSMomenta + 1]) -> Int_t {
  ...
  // Gets the number of non zero momenta in the array ignoring the highest
  // momenta
  ...
  SplitTLV(__OtherFSPiPlus4Momenta_MeV, OtherFSPiPlus4Momenta_MeV_X,
           OtherFSPiPlus4Momenta_MeV_Y, OtherFSPiPlus4Momenta_MeV_Z,
           OtherFSPiPlus4Momenta_MeV_T, NOtherFSPiPlus4Momenta_MeV);
  SplitTLV(__OtherFSProton4Momenta_MeV, OtherFSProton4Momenta_MeV_X,
           OtherFSProton4Momenta_MeV_Y, OtherFSProton4Momenta_MeV_Z,
           OtherFSProton4Momenta_MeV_T, NOtherFSProton4Momenta_MeV);
...
void TransversityVars::AddBranches(TTree *tree) {
  ...
  // First Proton
  tree->Branch("FirstProton_PDG", &FirstProton_PDG, "FirstProton_PDG/I");
...
void SetBranchAddressesTransversityVarsB(TTree *tree, TransversityVarsB *tvb) {
  ...
  if (kMaxProxies == _prxy.size()) {
    throw std::overflow_error(
        "Attempted to set up more than 100 TTrees with "
  ...
  tree->SetBranchAddress("IncNeutrino_4Mom_MeV",
                         &_prxy.back().IncNeutrino_4Mom_MeV);

  // Struck Nucleon
  tree->SetBranchAddress("StruckNucleonPDG", &tvb->StruckNucleonPDG);
  _prxy.back().StruckNucleon_4Mom_MeV = &tvb->StruckNucleon_4Mom_MeV;
  tree->SetBranchAddress("StruckNucleon_4Mom_MeV",
                         &_prxy.back().StruckNucleon_4Mom_MeV);
  ...
  tree->SetBranchAddress("HMTrackable_4Mom_MeV",
                         &_prxy.back().HMTrackable_4Mom_MeV);
  ...
  // deltaphiT
  tree->SetBranchAddress("deltaphiT_HMProton_deg",
                         &tvb->deltaphiT_HMProton_deg);

  // deltapT
  _prxy.back().deltapT_HMProton_MeV = &tvb->deltapT_HMProton_MeV;
  tree->SetBranchAddress("deltapT_HMProton_MeV",
                         &_prxy.back().deltapT_HMProton_MeV);

  // Deltap
  _prxy.back().Deltap_HMProton_MeV = &tvb->Deltap_HMProton_MeV;
  tree->SetBranchAddress("Deltap_HMProton_MeV",
                         &_prxy.back().Deltap_HMProton_MeV);
  ...
  // deltaalphaT
  tree->SetBranchAddress("deltaalphaT_HMProton_deg",
                         &tvb->deltaalphaT_HMProton_deg);
  ...
  tree->SetBranchAddress("HMProtonPion_3Mom_MeV",
                         &_prxy.back().HMProtonPion_3Mom_MeV);
  tree->SetBranchAddress("deltaphiT_HMProtonPion_deg",
                         &tvb->deltaphiT_HMProtonPion_deg);
  _prxy.back().deltapT_HMProtonPion_MeV = &tvb->deltapT_HMProtonPion_MeV;
  tree->SetBranchAddress("deltapT_HMProtonPion_MeV",
                         &_prxy.back().deltapT_HMProtonPion_MeV);
  tree->SetBranchAddress("deltaalphaT_HMProtonPion_deg",
                         &tvb->deltaalphaT_HMProtonPion_deg);
  _prxy.back().Deltap_HMProtonPion_MeV = &tvb->Deltap_HMProtonPion_MeV;
  tree->SetBranchAddress("Deltap_HMProtonPion_MeV",
                         &_prxy.back().Deltap_HMProtonPion_MeV);
